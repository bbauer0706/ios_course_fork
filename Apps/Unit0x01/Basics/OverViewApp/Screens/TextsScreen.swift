...
struct ShortsBlock: View {
    var body: some View {
        VStack(spacing: 5) {
            Text("– Weights and Shorts –").asLine
            HStack(alignment: .bottom) {
                Spacer()
                Text("Std")
...
struct MiscTextsBlock: View {
    ...
    // concerning "some View"
    //  - It is a concrete, composed, compiler-generated type.
    //  - It’s returned as some View, meaning the caller doesn’t need to know what it is.
    //  - This gives SwiftUI its performance and type safety without the runtime cost of type erasure.
    init () {
        // in extension View:
        //   func background<S>(_ style: S, [...more]) -> some View where S : ShapeStyle
        //   func foregroundColor(_ color: Color?) -> some View
        let text = Text("Hello")
        let bgText = text.background(Color.red)
        let fgText = bgText.foregroundColor(.white)

        print(" 1| type text: \(type(of: text))")
        print(" 2| type bgText: \(type(of: bgText))")
        print(" 3| type fgText: \(type(of: fgText))")
    }
    ...
    var body: some View {
            ...
            .padding(5)
            //.background(.pink.opacity(0.2))//.padding(5)
        }
    }
}
