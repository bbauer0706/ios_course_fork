...
class GameScene: SKScene {
    ...
    var gravity = CGVector(dx: 0, dy: -10)
    ...
    override func didMove(to view: SKView) {
        ...
        wall2 = SKSpriteNode(color: .orange, size: CGSize(width: 70, height: 10))
        ...
        wall3 = SKSpriteNode(color: .orange, size: CGSize(width: 70, height: 10))
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if let node = atPoint(location) as? SKSpriteNode, node == wall2 || node == wall3 {
            gravity = CGVector(dx: gravity.dy, dy: -gravity.dx)
            self.physicsWorld.gravity = gravity
            return
        }
